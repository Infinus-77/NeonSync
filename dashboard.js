  ...
  el.innerHTML = tasks
    .map((t) => {
      ...
      <div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid var(--border-subtle);cursor:pointer;"
        ...
        <div style="font-size:12px;font-weight:700;color:var(--blue);flex-shrink:0;">${pct}%</div>
      </div>
    `;
    })
    .join("");
...
function renderUpcomingDeadlines(tasks, now) {
  ...
  el.innerHTML = upcoming
    .map((t) => {
      ...
      return `
      <div style="display:flex;align-items:center;gap:10px;padding:9px 0;border-bottom:1px solid var(--border-subtle);cursor:pointer;"
        onclick="window.location.href='task-detail.html?id=${t.id}'"
...
function buildPodiumHTML(sorted, subtitleFn) {
  ...
  var restRows = sorted
    ...
    .map(function (u, i) {
      ...
      return (
        ...
        'border:1px solid var(--border-subtle);" data-testid="leader-' +
        u.id +
        '">' +
  ...
  var rest =
    sorted.length > 3
      ? '<div style="height:1px;background:var(--border-subtle);margin:16px 0 12px;"></div>' +
