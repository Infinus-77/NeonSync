...
export function showConfirm(message, confirmLabel = "Delete", danger = true) {
  return new Promise((resolve) => {
    ...
    overlay.innerHTML = `
      <div style="
        background:var(--bg-card,#111);border:1px solid rgba(255,255,255,0.1);
        border-radius:12px;padding:28px 32px;max-width:380px;width:90%;
        box-shadow:0 24px 64px rgba(0,0,0,0.6);
      ">
        <div style="font-size:14px;color:var(--text-primary,#fff);line-height:1.6;margin-bottom:22px;">
        ...
        <div style="display:flex;gap:10px;justify-content:flex-end;">
          <button id="ns-confirm-cancel" style="
            ...
            background:transparent;color:var(--text-muted,#aaa);cursor:pointer;font-size:13px;
          ">Cancel</button>
...
export function avatarStyle(size = 34) {
  return `width:${size}px;height:${size}px;border-radius:50%;background:linear-gradient(135deg,var(--accent-cyan),var(--accent-purple));display:flex;align-items:center;justify-content:center;font-size:${Math.floor(size * 0.38)}px;font-weight:600;flex-shrink:0;overflow:hidden;`;
}
