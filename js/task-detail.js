...
import {
  doc, getDoc, updateDoc, deleteDoc, addDoc, collection, query, where,
  onSnapshot, serverTimestamp, Timestamp, arrayUnion, getDocs
} from 'https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js';
import { statusBadge, priorityBadge, formatDate, timeAgo, getInitials, roleBadge, showToast, sanitizeHtml, showConfirm } from './utils.js';
...
requireAuth(async (user) => {
  ...
  usersSnap.docs.forEach(d => { allUsers[d.id] = { id: d.id, ...d.data() }; });
...
async function autoMarkOverdueSingle(t) {
  ...
  if (d < new Date()) {
    await updateDoc(doc(db, 'tasks', taskId), { status: 'overdue', updatedAt: serverTimestamp() }).catch(() => {});
  }
}
...
function renderTask(t) {
  ...
  document.getElementById('task-description').textContent = t.description || 'No description provided.';
  ...
  if (currentUser.role === 'member') {
    ...
    Array.from(statusSel.options).forEach(opt => { opt.disabled = flow.indexOf(opt.value) < current; });
  }

  const creator = allUsers[t.createdBy];
  document.getElementById('task-creator').textContent = creator ? creator.displayName : 'Unknown';
  ...
  const canEdit = currentUser.role === 'super_admin' || currentUser.role === 'admin' || t.createdBy === currentUser.id;
  document.getElementById('edit-task-btn').style.display = canEdit ? 'flex' : 'none';
  document.getElementById('delete-task-btn').style.display = currentUser.role === 'super_admin' ? 'flex' : 'none';

  if ((t.tags || []).length) {
    document.getElementById('tags-card').style.display = 'block';
    document.getElementById('task-tags-display').innerHTML = t.tags.map(tag =>
...
function renderRemarks(remarks) {
  ...
  document.getElementById('remarks-count').textContent = remarks.length ? `(${remarks.length})` : '';

  if (!remarks.length) {
    el.innerHTML = '<div class="empty-state" style="padding:16px;"><i class="ph ph-chat-circle"></i><p>No remarks yet</p></div>';
    return;
  }

  el.innerHTML = [...remarks].reverse().map(r => {
...
function renderAttachments(attachments) {
  ...
  if (!attachments.length) {
    el.innerHTML = '<div style="font-size:12px;color:var(--text-muted);">No attachments</div>';
    return;
  }
...
window.updateStatus = async (newStatus) => {
  ...
  try {
    ...
    if (taskData.createdBy !== currentUser.id) {
      await createNotification(taskData.createdBy, 'status_update',
        `Task "${taskData.title}" status changed to ${newStatus} by ${currentUser.displayName}`, taskId);
    }
...
window.saveCompletion = async () => {
  ...
  try {
    ...
    await addTaskLog('percentage_update', String(taskData?.completionPercentage || 0), String(pct));
    await writeActivityLog('percentage_update'); // ✅ feeds heatmap

    if (pct === 100 && taskData?.createdBy && taskData.createdBy !== currentUser.id) {
      await createNotification(taskData.createdBy, 'status_update',
        `Task "${taskData.title}" was marked 100% complete by ${currentUser.displayName}`, taskId);
    }
...
window.addRemark = async () => {
  ...
  try {
    const remark = { userId: currentUser.id, message: msg, timestamp: new Date().toISOString() };
    await updateDoc(doc(db, 'tasks', taskId), { remarks: arrayUnion(remark), updatedAt: serverTimestamp() });
    ...
    for (const uid of (taskData?.assignedTo || [])) {
      if (uid !== currentUser.id) {
        await createNotification(uid, 'remark_added',
          `${currentUser.displayName} added a remark on "${taskData?.title}"`, taskId);
      }
    }
    if (taskData?.createdBy && taskData.createdBy !== currentUser.id && !(taskData?.assignedTo || []).includes(taskData.createdBy)) {
      await createNotification(taskData.createdBy, 'remark_added',
        `${currentUser.displayName} added a remark on "${taskData?.title}"`, taskId);
    }

    document.getElementById('remark-text').value = '';
    showToast('Remark added', 'success');
  } catch (err) { showToast('Failed to add remark', 'error'); }
};
...
window.addAttachment = async () => {
  ...
  try {
    const att = { fileName: name, fileURL: url, uploadedBy: currentUser.id, timestamp: new Date().toISOString() };
...
function renderMessages(msgs) {
  ...
  if (!msgs.length) {
    el.innerHTML = '<div class="empty-state" style="padding:16px;"><i class="ph ph-chat-circle"></i><p>No messages yet</p></div>';
    return;
  }
...
window.sendTaskMessage = async () => {
  ...
  try {
    await addDoc(collection(db, 'messages'), {
      chatId: taskChatId, senderId: currentUser.id,
...
function renderLogs(logs) {
  ...
  if (!logs.length) {
    el.innerHTML = '<div class="empty-state" style="padding:12px;"><i class="ph ph-list-bullets"></i><p>No activity yet</p></div>';
    return;
  }

  const icons = { status_change: 'ph-arrows-clockwise', remark_added: 'ph-chat-circle-text', percentage_update: 'ph-chart-line', task_updated: 'ph-pencil', default: 'ph-activity' };
...
async function addTaskLog(actionType, previousValue, newValue) {
  try {
    await addDoc(collection(db, 'taskLogs'), {
      taskId, updatedBy: currentUser.id,
      actionType, previousValue, newValue,
      timestamp: serverTimestamp()
    });
  } catch (_) {}
}
...
window.openEditModal = () => {
  ...
  document.getElementById('edit-priority').value = taskData.priority || 'medium';
  if (taskData.deadline) {
    const d = taskData.deadline.toDate ? taskData.deadline.toDate() : new Date(taskData.deadline);
    document.getElementById('edit-deadline').value = d.toISOString().slice(0, 16);
  }
...
window.deleteTask = async () => {
  const confirmed = await showConfirm('Delete this task permanently? This cannot be undone.', 'Delete');
  if (!confirmed) return;
  try {
