...
import {
  ...
  getDocs, orderBy, serverTimestamp, setDoc
} from 'https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js';
import {
  getInitials, roleBadge, timeAgo, showToast, sanitizeHtml, showConfirm
} from './utils.js';

// ─── FIX: Use a secondary Firebase app to create users without signing out current admin ───
import { initializeApp, getApps } from 'https://www.gstatic.com/firebasejs/10.7.1/firebase-app.js';
import { getAuth, createUserWithEmailAndPassword, signOut as secondarySignOut } from 'https://www.gstatic.com/firebasejs/10.7.1/firebase-auth.js';
...
requireAuth(async (user) => {
  ...
  if (user.role !== 'super_admin' && user.role !== 'admin') {
    window.location.href = '../public/dashboard.html'; return;
  }
  ...
  document.getElementById('users-subtitle').textContent =
    user.role === 'super_admin' ? 'Manage all team members and roles' : 'Manage your team members';
...
window.filterUsers = () => {
  ...
  if (currentUser.role === 'admin') {
    users = users.filter(u => u.role !== 'super_admin' || u.id === currentUser.id);
  }
    ...
    return `<tr data-testid="user-row-${u.id}">
      ...
      <td>
        <div style="display:flex;gap:6px;">
          <a href="../public/profile.html?uid=${u.id}" class="btn btn-secondary btn-sm" title="View profile"><i class="ph ph-eye"></i></a>
          ${canManage ? `
...
window.submitUser = async (e) => {
  ...
  btn.disabled = true;
  btn.innerHTML = '<i class="ph ph-circle-notch" style="animation:spin 0.8s linear infinite;"></i> Creating...';

  try {
    if (currentUser.role === 'admin' && role === 'super_admin') {
      showToast('Admins cannot create Super Admins', 'error');
      btn.disabled = false;
      btn.innerHTML = '<i class="ph ph-user-plus"></i> <span id="cu-submit-label">Create User</span>';
      return;
    }
    ...
    await setDoc(doc(db, 'users', cred.user.uid), {
      displayName: name, email, role, bio,
      skills: [], photoURL: '',
    ...
    await createNotification(cred.user.uid, 'profile_created',
      `Welcome to NeonSync, ${name}! Your account has been created as ${role}.`, null);
...
window.confirmRoleChange = async () => {
  ...
  if (currentUser.role === 'admin' && newRole === 'super_admin') {
    showToast('Admins cannot assign Super Admin role', 'error'); return;
  }

  try {
    await updateDoc(doc(db, 'users', roleChangeUserId), { role: newRole, updatedAt: serverTimestamp() });
    await createNotification(roleChangeUserId, 'role_changed',
      `Your role has been updated to ${newRole} by ${currentUser.displayName}`, null);
...
window.confirmDeleteUser = async (uid, name) => {
  const confirmed = await showConfirm(`Remove "${name}"? Their tasks will remain but their profile will be deleted.`, 'Remove User');
